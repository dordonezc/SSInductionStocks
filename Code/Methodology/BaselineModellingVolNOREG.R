...
## Twitter
with(all_data, ccf(Ehat, PolarityTwitter, na.action = na.exclude))

## SA
with(all_data, ccf(Ehat, PolaritySA, na.action = na.exclude))

## WSJ
with(all_data, ccf(Ehat, PolarityWSJ, na.action = na.exclude))
...
mod_glm <- glm(y_reg[-1] ~ x_reg[-1, 1:2], family = "gaussian",
...
reg_data_f <- reg_data %>% select(1:3) 
...
all_preds %>% arrange(name, holdout_id) %>% 
  group_by(name) %>%  
  summarise(mean((Actual-value)^2))


write_csv(all_preds, "Results/Volatility/BaselineSentimentNoRegsLast3.csv")
